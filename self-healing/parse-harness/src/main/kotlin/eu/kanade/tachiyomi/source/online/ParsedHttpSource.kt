...
/** Selector-driven base — the standard Tachiyomi behavior, reproduced for JVM. */
abstract class ParsedHttpSource : HttpSource() {

    override fun popularMangaParse(response: Response): MangasPage {
        val document = response.asJsoup()
        val mangas = document.select(popularMangaSelector()).map { popularMangaFromElement(it) }
        val hasNext = popularMangaNextPageSelector()?.let { document.selectFirst(it) } != null
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        ...
        val hasNext = searchMangaNextPageSelector()?.let { document.selectFirst(it) } != null
        return MangasPage(mangas, hasNext)
    }
    ...
    override fun latestUpdatesParse(response: Response): MangasPage {
        ...
        val hasNext = latestUpdatesNextPageSelector()?.let { document.selectFirst(it) } != null
        return MangasPage(mangas, hasNext)
    }
